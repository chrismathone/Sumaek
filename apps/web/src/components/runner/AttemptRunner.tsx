 ...
 *
 * 이 카운트다운은 **표시용이다** (T6.1). 마감을 정하는 것은 서버이고, 판정은
 * DB 시계로 한다 — `saveResponse`가 `now()`와 `started_at + time_limit`을 같은
 * 질의에서 비교하고, 지났으면 저장을 거부하며 그 자리에서 자동 제출한다.
 * 따라서 탭을 닫아 타이머를 멈추거나 기기 시계를 되돌려도 마감은 그대로다.
 * 여기서 하는 자동 제출은 **더 빨리 알리기 위한 것**이지 강제가 아니다.
 * ───────────────────────────────────────────────────────────── */
