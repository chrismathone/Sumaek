...
export default async function LearnTodayPage() {
  ...
  const { active: activeStep, verdict } = readDay({
    ...
    /* 단계 상태를 여기서 덮어쓰지 않는다 (T1.4·T4.1).
     *
     * conceptState·testState·reviewState는 이 화면이 자료·배정 행을 직접
     * 세어 만든 값이고, planToDayInput이 낸 값은 **서버가 확정한 계획**에서
     * 나온다. 둘을 겹치면 화면의 「다 마쳤습니다」와 서버의 완료 기록이
     * 서로 다른 근거를 갖는다 — 그리고 그 기록이 곧 completed_at이다.
     * 위 세 값은 카드 안의 배지·건수에만 쓴다. */
  });
