...
export interface ExtractionProfile {
  ...
  fonts: {
    ...
    unitNumber: RegExp;
    /**
     * **인라인 분수** 글꼴. 분수를 한 글자로 담는데(`;2!;` = 1/2), 두 줄
     * 높이라 기준선이 본문보다 아래로 내려간다 — 문항 0049 선택지 ④에서
     * 본문 줄은 y1=275.0인데 분수는 280.5였다. 허용 오차를 넘어 다른 줄로
     * 갈리면 **곱셈 기호와 분수가 따로 모여**, 선택지가
     * `××××= 1/2 1/2 1/2 …`처럼 순서째 뒤집힌다.
     *
     * 수식 글꼴 전체에 적용하면 안 된다 — 세로셈 표의 행까지 한 줄로 뭉친다.
     */
    inlineFraction: RegExp;
  };
  ...
  figures: {
    ...
    minHeight: number;
    /**
     * 선이 성긴 도형 — **좌표평면 그래프가 여기 걸린다.**
     *
     * 축 둘에 직선 하나면 벡터가 다섯도 안 되는데, minDrawings로는 문턱을
     * 못 넘어 도형으로 서지 않는다. 도형이 안 서면 그 안의 라벨(`y=3x+6`·
     * `O`·`x`)을 걸러 낼 근거가 없어 전부 발문으로 샌다 — 여섯 권에서
     * 1067문항(17.3%)이 그랬다. 렌더는 성공하므로 렌더 검사로는 안 잡힌다.
     *
     * 문턱만 낮추면 **본문 안의 계산 과정 블록**까지 도형이 되어 발문이
     * 사라진다(중1-1 0444 「다음 계산 과정에서 ㉠, ㉡에…」가 254자에서
     * 37자로 줄었다). 가름선은 **폭**이다 — 그래프는 단 폭의 3분의 1쯤인
     * 70~80pt인데 계산 블록은 단을 가득 채운다.
     */
    sparse: {
      minDrawings: number;
      /** 이 폭을 넘는 성긴 뭉치는 도형으로 보지 않는다(pt) */
      maxWidth: number;
    };
  };
}
