/* ─────────────────────────────────────────────────────────────
 * 교재별 추출 프로파일
 *
 * 문제집마다 조판이 다르다. 단 수, 문항 번호의 폰트·크기, 수식 폰트 계열,
 * 선택지 기호, 유형 머리글의 생김새가 전부 다르다. 그걸 파서 본문에
 * 흩어 두면 두 번째 교재를 붙일 때 첫 교재가 깨진다.
 *
 * 그래서 "이 교재는 이렇게 생겼다"를 한 곳에 모아 둔다. 파서는 프로파일을
 * 읽고 움직일 뿐 특정 교재를 모른다.
 *
 * 프로파일은 코드에만 두지 않고 반입 때 DB(book_editions.extraction_profile)에도
 * 적는다 — 어떤 규칙으로 뽑은 문항인지가 나중에 반드시 필요해진다.
 * ───────────────────────────────────────────────────────────── */

export interface ExtractionProfile {
  /** 안정적인 식별자 — DB에 적히는 값 */
  id: string;
  /** 이 프로파일을 고칠 때마다 올린다. 문항에 함께 기록된다. */
  version: string;
  label: string;
  /** 어떤 교재에 쓰는가 (사람이 읽는 설명) */
  appliesTo: string;

  layout: {
    /** 본문 단 수 */
    columns: number;
    /** 위·아래 여백 비율 — 이 안의 span은 머리글·꼬리말로 본다 */
    topMarginRatio: number;
    bottomMarginRatio: number;
    /** 같은 줄로 묶을 기준선 허용 오차(pt) */
    lineToleranceY: number;
  };

  fonts: {
    /** 수식 폰트 — 이 폰트의 span은 수식으로 해독한다 */
    math: RegExp;
    /**
     * 도형 **안**의 글자 전용 글꼴 — 꼭짓점 이름·치수·각도.
     *
     * 그림 영역을 벡터 뭉치로 알아내는 규칙(figures)이 이미 있지만, 선이
     * 성긴 도형은 뭉치로 안 잡힌다. 그러면 라벨이 발문 한가운데로 새어
     * 「다음 그림에서 △ABC가 A x 58° B C 이등변…」이 된다(중2-2 도형
     * 단원에서 947개가 이 글꼴로 온다). 글꼴 자체가 「그림 안」이라는
     * 뜻이므로 기하와 무관하게 라벨로 뺀다.
     */
    figureLabel: RegExp;
    /** 문항 번호 */
    questionNumber: { font: RegExp; minSize: number };
    /** 선택지 기호 ①②③④⑤ */
    choiceMarker: RegExp;
    /** 유형 머리글: 라벨·번호·제목 */
    typeLabel: RegExp;
    typeNumber: RegExp;
    typeTitle: RegExp;
    /** 교과서 참조 («개념원리 중학 수학 1-1 32쪽») */
    textbookRef: RegExp;
    /** 보기·조건 박스의 라벨 */
    conditionLabel: RegExp;
    /** 쪽 번호 — 이 폰트가 든 줄은 러닝헤드다 */
    pageNumber: RegExp;
    /** 러닝헤드의 중단원 번호 — 「02 최대공약수와 최소공배수」 */
    unitNumber: RegExp;
    /**
     * **인라인 분수** 글꼴. 분수를 한 글자로 담는데(`;2!;` = 1/2), 두 줄
     * 높이라 기준선이 본문보다 아래로 내려간다 — 문항 0049 선택지 ④에서
     * 본문 줄은 y1=275.0인데 분수는 280.5였다. 허용 오차를 넘어 다른 줄로
     * 갈리면 **곱셈 기호와 분수가 따로 모여**, 선택지가
     * `××××= 1/2 1/2 1/2 …`처럼 순서째 뒤집힌다.
     *
     * 수식 글꼴 전체에 적용하면 안 된다 — 세로셈 표의 행까지 한 줄로 뭉친다.
     */
    inlineFraction: RegExp;
  };

  patterns: {
    /** 문항 번호 형태 (인접 span을 붙인 뒤 검사) */
    questionNumber: RegExp;
    choiceMarker: RegExp;
    conditionLabel: RegExp;
    textbookRef: RegExp;
    /** 꼬리말의 단원 표기 */
    runningHead: RegExp;
    /** 구매자 식별 워터마크 (이메일 등) — 문항이 아니며 저장해서도 안 된다 */
    purchaserStamp: RegExp;
    /** 여러 문항이 공유하는 지시문의 번호 구간 — 캡처 그룹 2개 (from, to) */
    sharedInstruction: RegExp;
  };

  figures: {
    /** 이만큼 이상 뭉친 벡터 도형만 「그림」으로 본다 */
    minDrawings: number;
    /** 뭉치로 묶는 거리(pt) */
    clusterGap: number;
    /** 이보다 작은 뭉치는 밑줄·괄호 같은 장식으로 본다 */
    minWidth: number;
    minHeight: number;
    /**
     * 선이 성긴 도형 — **좌표평면 그래프가 여기 걸린다.**
     *
     * 축 둘에 직선 하나면 벡터가 다섯도 안 되는데, minDrawings로는 문턱을
     * 못 넘어 도형으로 서지 않는다. 도형이 안 서면 그 안의 라벨(`y=3x+6`·
     * `O`·`x`)을 걸러 낼 근거가 없어 전부 발문으로 샌다 — 여섯 권에서
     * 1067문항(17.3%)이 그랬다. 렌더는 성공하므로 렌더 검사로는 안 잡힌다.
     *
     * 문턱만 낮추면 **본문 안의 계산 과정 블록**까지 도형이 되어 발문이
     * 사라진다(중1-1 0444 「다음 계산 과정에서 ㉠, ㉡에…」가 254자에서
     * 37자로 줄었다). 가름선은 **폭**이다 — 그래프는 단 폭의 3분의 1쯤인
     * 70~80pt인데 계산 블록은 단을 가득 채운다.
     */
    sparse: {
      minDrawings: number;
      /** 이 폭을 넘는 성긴 뭉치는 도형으로 보지 않는다(pt) */
      maxWidth: number;
    };
    /**
     * 글자가 이만큼 안에 있어야 그림의 라벨로 본다(pt) — **선까지의 거리다.**
     *
     * 뭉치의 바깥 사각형으로 재면 안 된다. 뭉치는 사각형이 아니다. 좌표평면
     * 격자(x=217~302) 옆에 답 쓰는 칸 선(x=65~203) 하나가 clusterGap 안으로
     * 들어오면 둘이 한 뭉치가 되고, 바깥 사각형은 **단을 가로지른다**(너비
     * 237). 그 띠에 걸친 발문은 통째로 라벨이 된다 — 중1-1 0918이 그렇게
     * 화면에서 빈 문항이 됐다.
     *
     * 그래서 뭉치 안의 **선 하나하나**에 대고 잰다. 치수 라벨은 변 바깥으로
     * 반쯤 나오지만 선에서 멀지는 않다.
     */
    labelMargin: number;
  };
}
