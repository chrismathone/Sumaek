...
export type Run =
  ...
  | {
      ...
      unknown: string[];
      /**
       * 어느 글꼴로 읽었는가 — **원문 전체로 다시 읽을 때 필요하다.**
       *
       * 부분집합 글꼴이라 코드는 글꼴 안에서만 뜻이 있다. 별책은 수식을
       * 조각으로 흘려 보내고 조각마다 따로 읽어 이어 붙이는데, 그러면
       * 나중에 온 표시가 앞 조각을 못 고친다. 원문을 통째로 다시 읽으면
       * 해결되지만 그러려면 글꼴을 알아야 한다.
       */
      font?: string;
    };
