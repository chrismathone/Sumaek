...
describe("개념 → 정본 매핑 표", () => {
  it("1단원 개념 9자리가 전부 표에 있다", () => {
    /* 열쇠에 쪽이 들어간다 — 소단원 제목의 (1)·(2)가 다른 글꼴이라
     * 추출된 제목에서 빠져, 쪽이 없으면 서로 다른 개념이 겹친다. */
    const keys = [
      [10, "소인수분해", "1"], [10, "소인수분해", "2"], [11, "소인수분해", "3"],
      [17, "소인수분해를 이용하여 약수 구하기", "1"],
      [30, "공약수와 최대공약수", "1"], [30, "공약수와 최대공약수", "2"],
      [35, "공배수와 최소공배수", "1"], [35, "공배수와 최소공배수", "2"],
      /* 개념3 「최대공약수와 최소공배수의 관계」 — 제목이 질문형이 아니라
       * 예전 파서가 통째로 놓쳤고, 그래서 표에도 없었다. */
      [35, "공배수와 최소공배수", "3"],
    ] as const;
    for (const [page, sub, no] of keys) {
      expect(
        KWR_M11_CH1_TARGETS.get(conceptTargetKey(page, sub, no)),
        `${page}|${sub}|${no}`,
      ).toBeDefined();
    }
  });
