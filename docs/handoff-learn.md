...
- **감사 기록**: `audit_events`의 `content.rights-confirm`(before/after 상태) ·
  ...
  `question.quarantine` 2건(4.4절). 전부 actor `demo-teacher`, 사유에 소유자 지시 명시.
...
|---|---|
| 1. 개념에 정렬된 문항 | **213** |
| 2. + `review_status = 'published'` | **208** |
| 3. + `content_rights.status = 'usable'` | **208** |

213 = published 208 + 격리 2 (4.4절) + 그림 검수 대기 3.

### 4.3 이 수치를 다시 뽑는 SQL — 기대값 213 · 208 · 208
...
### 4.4 격리 2건 — 풀 수 없는 문항 (원칙 12)

일괄 검수 통과 직후 학생 연습 화면에서 눈으로 확인해 **핵심 수식이 아예 탈락해
풀 수 없는 문항 2건**을 `quarantined`로 되돌렸다 (`quarantine_reason` +
`question.quarantine` 감사 기록):

| 인쇄번호 | id | 무엇이 깨졌나 |
|---|---|---|
| #0106 | `019fc367-455d-7010-8760-483938b346f8` | 「두 자연수의 최대공약수가 __의 공약수를 모두 구하시오」 — 최대공약수 값 수식 탈락 |
| #0120 | `019fc367-4f41-740e-ab44-7acab5f8daf7` | 최소공배수 값 탈락 + 다음 묶음 머리글 「[0121~0124] …」가 본문에 혼입 |
...
**자동 선별의 성질 둘 — 결함이 아니라 다음 큐레이션 대상:**

1. **세트 간 중복.** 한 문항이 여러 개념에 정렬돼 있어(4.1절의 379 > 213) 소인수분해
   연습과 약수와 약수의 개수 연습이 지금 같은 소수 판별 문항들을 공유한다. 교사가
   자료에 `question_ids`를 지정하면(큐레이션 경로) 해소된다.
2. **`created_at` 순 = 교재 앞쪽부터.** 쉬운 준비 문항만 나온다. 난이도 안배도
   큐레이션 몫이다.

### 4.6 내용 검수는 아직 안 됐다 — 일괄 개방의 그림자

4.2절의 일괄 `published`는 **형식 게이트를 연 것이지 문항 내용 검수가 아니다.**
화면 표본(연습 세트 25칸)만 훑었는데도 다음이 나왔다 — **전수 검수가 남은 일이다**:

- 글자 파편(교재 머리글·단원명 글자가 본문에 섞임, 풀 수는 있다): `#0184` ·
  `#0198` · `#0211` (published 210건 스캔 — ≤2자 text-run 4연속 휴리스틱 + 눈 확인)
- 문장 결손: 최대공약수 연습 1번이 「두 수 18과 24음을 구하시오」로 나온다 (「에 대하여 다음을」의 일부 탈락 의심)
- **지시문 상실**: `#0121~0124`의 공동 지시문(「다음 수들의 최소공배수를 소인수의
  곱으로 나타내시오」)이 격리된 `#0120` 본문에 붙어 있어, 네 문항이 지시문 없이
  숫자쌍만 낸다 — 추출 단계의 묶음 지시문 분리 문제([handoff.md](handoff.md) 7절 계보)

다시 찾는 휴리스틱(2026-08-04 스캔에 쓴 것): ① 인접 text-run에서 조사(가/는/이)+공백
뒤가 「의/인/일 」로 시작 → 수식 탈락(이번에 2건 전부 이걸로 나왔다) ② 본문에
`[0NNN~0NNN]` → 머리글 혼입 ③ ≤2자 text-run 4연속 → 글자 파편.
