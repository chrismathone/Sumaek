...
- [~] `pnpm boundary:check`, `lint`, `typecheck`, `test`, `build`, recovery **통과**. recovery는 107건 위반에서 0건으로 되돌렸다(아래). **E2E는 2건이 남는다** — 아래 「남는 실패」
...
남긴 둘은 **데모 콘텐츠 상태**에 달렸고, 그 영역은 병행 세션(RPM)이 개념·문항 정렬을 손보는 중이라 건드리지 않기로 한 곳이다. 스펙을 데이터에 맞춰 약하게 고치는 것은 통과시키는 것이지 검증하는 것이 아니다 — 콘텐츠가 자리를 잡은 뒤에 고친다.
