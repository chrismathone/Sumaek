...
- [~] `pnpm boundary:check`, `lint`, `typecheck`, `test`, `build`, recovery **통과**. recovery는 107건 위반에서 0건으로 되돌렸다(아래). 자율 E2E는 세 폭 23통과. **표준 E2E desktop은 44통과 3실패** — 아래 「남는 실패」
...
**남는 실패 (정직하게)**: 표준 E2E desktop에서 4건이 실패하고 있었다. **T6.4가 만든 것이 아니다** — main에서도 같은 자리에서 실패한다(직접 확인). 둘은 고쳤고 둘은 남긴다.
...
| `teacher-app` 스모크 | 명단 링크의 접근 이름이 비어 있어(이름·학년·점수가 각각 `<p>`) `getByRole("link", {name})`이 못 찾는다 | **부분 고침** — 명단 구역으로 스코프해 통과시켰으나, 뒤이은 「가감법 자료 게시됨」 단언에서 다시 실패한다 |
| `route-builder` 학생 오버라이드 | 「건너뛸 노드가 기준 루트 버전에 없습니다」 — 시드 루트에 스펙이 기대하는 노드(가감법·일차방정식 복습)가 없다 | **남김** |
| `materials` 자료 왕복 | 가감법 개념의 자료·진도 상태에 달렸다. 실행 순서에 따라 통과·실패가 갈린다 | **남김** |

남긴 셋은 **데모 콘텐츠 상태**에 달렸고, 그 영역은 병행 세션(RPM)이 개념·문항 정렬을 손보는 중이라 건드리지 않기로 한 곳이다. 스펙을 데이터에 맞춰 약하게 고치는 것은 통과시키는 것이지 검증하는 것이 아니다 — 콘텐츠가 자리를 잡은 뒤에 고친다.
