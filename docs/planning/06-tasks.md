...
| ~~G-09~~ **닫힘** | 시험 제한 시간이 클라이언트에서만 강제됨 | `saveResponse`가 DB 시계로 `started_at + time_limit`을 검사하고, 지났으면 거부한 뒤 그 자리에서 자동 제출한다 | P1 | T6.1 ✔ |
| ~~G-10~~ | ~~기존 full-loop가 시드된 평가 소비만 검증~~ | 빈 조직 생성부터 실제 워커까지 전체 왕복 검증 **(T6.2 닫힘)** | P0 | T6.2 |
...
### [x] Phase 6, T6.1: 시험 제한 시간 서버 강제 RED→GREEN
...
**산출물**:
- `apps/web/src/lib/domain/attempt.ts` — `saveResponse`의 DB 시계 마감 검사와 자동 제출
- `apps/web/src/components/runner/AttemptRunner.tsx` — 「서버가 검사하지 않는다」는 낡은 주석 정정
- `apps/web/test/integration/attempt-deadline.test.ts` (7)

**인수 조건**:
- [x] 탭 종료·기기 시계 변경으로 마감을 우회할 수 없음 — 판정이 `now()`와 `started_at`을 **같은 질의**에서 비교한다. 클라이언트가 보낼 수 있는 시간 값이 이 경로에 없다
- [x] 마지막 정상 저장 답안이 보존된 채 제출됨 — 늦은 답안은 저장하지 않고 자동 제출만 한다
- [x] 중복 자동/수동 제출이 1회 전이로 끝남 — `submitAndGrade`의 기존 CAS가 그대로 받는다
- [x] 신규/변경 모듈 커버리지 80% 이상 — `attempt.ts` 문 92% (이 파일의 테스트 5벌 합산)

**설계 판단**: 마감이 지나면 **거부하고 그 자리에서 제출**한다. 거부만 하고 열어 두면 응시가 `in_progress`로 영영 남는다 — 학생은 「제출됨」을 못 보고 교사 현황판에는 미제출로 남는다.

제출 경로는 마감을 이유로 막지 **않는다**. 마감은 더 쓰지 못하게 하는 것이지 낸 것을 버리는 것이 아니다 — 막으면 학생이 푼 답안이 영영 채점되지 않는다.
...
### [x] Phase 6, T6.2: 빈 워크스페이스→학생 하루 완료 실워커 E2E RED→GREEN
...
**인수 조건**:
- [x] 시드된 조직·과정 기간·평가에 의존하지 않음 — 픽스처는 조직·원장 계정·문제은행만 놓는다 (제품에 만들 화면이 없는 셋). 개념은 공용 교육과정에서 고른다
- [x] 교사가 평가 생성 버튼을 누르지 않아도 학생에게 테스트가 나타남 — ④는 클릭이 하나도 없는 테스트다. 가짜 워커로 바꿔 실패를 확인했다
- [x] 학생 완료가 교사 현황과 다음 일정 변경안에 반영됨 — ⑥(완료 1·기록 없음 1·막힘 0) · ⑦(못 나감 마감 → 확인테스트 이동 → 승인 대기 1건)
- [x] 테스트 종료 후 데모 데이터와 불변 증거를 오염시키지 않음 (`pnpm purge:test-data`로 되돌아감) — 조직 단위 회수를 더했다. 응시 기록이 남은 조직은 지울 수 없어 **보관 처리**하고 무엇이 막았는지 표 이름까지 출력한다
- [x] `--repeat-each=5`가 desktop에서 5/5 통과

**T6.2에서 드러나 고친 제품 결함 넷**:
1. **온보딩이 막다른 길** — 루트 게시는 준비도 게이트가 자료를 요구하는데 설정 순서는 자료를 루트 **뒤**에 두고 루트를 선행으로 요구했다. 새 학원은 「루트 게시」를 할 차례로 받아 들고 거부당하는데 자료 단계는 차단돼 링크조차 없었다
2. **「하러 가기」가 폼 없는 화면으로** — 과정 기간·반·학생을 만드는 폼은 `/app/settings`에만 있는데 세 단계가 각각 목록 화면을 가리켰다. 계정은 발급 화면이 따로 있는데도 목록을, 준비도는 T5.4가 만든 화면 대신 루트 목록을 가리켰다
3. **생성된 평가가 자기를 부른 노드를 모른다** — `assessment_instances.route_node_id`를 채우는 코드가 없어 평가 노드가 영영 「생성되지 않았습니다」로 막혔다. 필수 항목이 막힌 하루는 완료될 수 없다 (실측: 운영 DB 1884건 중 0건)
4. **하루의 첫 투영이 겹치면 500** — `for update`는 없는 행을 잠그지 못해 둘 다 insert하고 유니크 인덱스가 뒤엣것을 막았다. 학생이 「오늘 학습」을 처음 여는 순간이 그 지점이다

**치르는 값 (숨기지 않는다)**: 실행마다 조직이 하나씩 **보관 상태로 남는다.** 학생이 응시하면 `grade_decisions`가 `attempts`를 붙들고 거기서 사슬이 이어져 삭제가 막히기 때문이다(ADR-0015 · 불변 조건 15). 빈 학원이어야 하므로 조직을 재사용할 수도 없다. 남는 것은 `status='archived'`로 내려 목록에서 빠지고, `pnpm purge:test-data`가 몇 건인지와 무엇이 막았는지를 표 이름까지 출력한다. 데모 워크스페이스는 건드리지 않는다 — 실측으로 학습자·반·자료·기간·문항 전부 0건이다.
...
### [x] Phase 6, T6.3: 동시성·부하·모바일·접근성 검증 RED→GREEN
...
**인수 조건**:
- [x] 중복 완료·생성·마감이 각각 1회만 반영됨 — 셋을 각각 10개씩 겹쳐 부른다(`packages/db/test/concurrent-commands.test.ts`). 완료·마감은 이미 튼튼했고, **생성은 예외로 죽고 있었다**(아래)
- [x] 30명 동시 제출에서 오류율과 p95가 합의된 SLO 이내임 — 유실 0 · 예외 0 · 채점 결정 응시당 1건 · p50 590ms · p95 623ms. **단, 도메인 경로만 잰 값이다**(HTTP·서버 액션 제외) — L-02의 하한으로만 읽는다
- [x] desktop·tablet·mobile 전부 통과 — 자율 여정 전체를 세 폭에서 돈다(1440 · iPad/WebKit · 360px)
- [x] axe A/AA 위반 0건, 키보드만으로 완주 가능 — 학생 하나가 클릭 없이 로그인→자료→응시→제출까지 간다
- [x] 신규/변경 모듈 커버리지 80% 이상 — `assessment-generation.ts` 86.6% · `session-execution.ts` 100% · `learner-day-plan.ts` 87.1%

**T6.3에서 드러나 고친 제품 결함**: **겹쳐 들어온 평가 생성이 예외로 죽었다.** 「이미 있으면 그대로 반환」이 잠금 없는 select 하나뿐이라 둘이 동시에 들어오면 둘 다 넣고, 유니크 인덱스가 뒤엣것을 막고, 그 예외가 그대로 올라갔다. 워커에서는 작업 실패 → 재시도 5회 소진 → DLQ → E-17이 되어, 교사 업무함에 **이미 만들어져 있는** 시험을 두고 「생성 실패」가 뜬다. 여러 워커·lease 회수·교사의 수동 생성이 겹치는 경로는 드물지 않다.

**아는 한계 (숨기지 않는다)**:
- **k6는 아직 못 돌린다.** 바이너리도 스테이징도 없다. `scripts/load/autonomous-day.k6.js`는 작성만 돼 있고(세션 쿠키만 있으면 실행 가능), 목표 RPS(2,000) 실측은 스테이징이 서야 한다. 지금 재고 있는 것은 **동시성**이지 **규모**가 아니다.
- **WebKit에서는 키보드 완주를 재지 않는다.** Safari가 기본 설정에서 링크를 탭 순서에 넣지 않기 때문이다(실측: Chromium `A[href]>INPUT>…` / WebKit `INPUT>BUTTON>…`). 페이지가 바꿀 수 있는 것이 아니라 거기서 실패하면 엔진을 재는 것이 된다. 폼 요소는 WebKit에서도 탭으로 닿는다.
...
### [x] Phase 6, T6.4: 운영 모니터링·런북·릴리스 문서 갱신 RED→GREEN
...
**인수 조건**:
- [x] 교사가 수업 시작 전 자동 평가 누락과 차단 학생을 발견할 수 있음 — 교사는 `/app/readiness`(T5.4), 운영자는 `pnpm worker:status -- --flow`. 둘이 **같은 투영기**를 보므로 화면과 명령이 다른 말을 하지 않는다
- [x] 운영자가 적체 원인과 안전한 재실행 방법을 확인할 수 있음 — RB-16이 파이프라인 다섯 칸을 앞에서부터 배제하게 하고, 생성 실패 여섯 사유마다 고치는 화면을 붙였다
- [x] `pnpm boundary:check`, `lint`, `typecheck`, `test`, `build`, E2E, recovery 검사가 모두 통과함 — recovery는 107건 위반에서 0건으로 되돌렸다(아래). 남아 있던 E2E·db 실패 넷은 병행 세션(RPM)이 콘텐츠를 마친 뒤 전부 정리했다 — 아래 「뒤늦게 정리한 것」
- [x] README의 핵심 순환이 실제 코드·테스트와 일치함 — 한 줄 화살표를 그림으로 바꾸고 학생 하루·교사 현황·마감·변경안을 넣었다. 「[워커]로 표시한 두 칸은 사람이 누르지 않는다」를 못 박았다
- [x] 신규/변경 모듈 커버리지 80% 이상 — `status/autonomous-flow.ts` 89.8%(함수 100%)

**T6.4에서 드러나 고친 것**:
1. **릴리스 게이트가 늘 빨간불이었다** — `pnpm verify:recovery`가 위반 107행으로 실패하고 있었고, 넷을 따라가 보니 **전부 우리 테스트가 남긴 것**이었다(제품 결함 없음). 픽스처가 `published_at` 없이 게시 상태를 만들고 가짜 체크섬을 넣었고(I-08 22건), 응시 행을 직접 넣어 미제출 응시가 쌓였고(I-09 7그룹·최대 22건), 자율 워크스페이스 정리가 파생만 지워 증거를 고아로 만들었고(I-11 32건), 생성 실패 테스트가 jobs만 지우고 outbox를 남겼다(R-04 46건). 원인을 각각 고치고 쌓인 것은 `pnpm purge:invariant-residue`가 치운다
2. **장애 때 치는 kill switch 이름 다섯이 코드에 없었다** — `auto_question_publish`·`auto_schedule_recalc`·`formula_auto_repair`·`external_notification`·`curriculum_release_publish` (문서 48곳). 여섯째 `ai_provider:<name>`은 이름이 아니라 **구현이 없어서**, 지우지 않고 `KILL_SWITCH_KEYS_WITHOUT_IMPLEMENTATION`에 적었다. 다시 어긋나지 않게 `kill-switch-docs-sync.test.ts`가 문서와 코드를 대조한다
3. **불변 조건 개수 문구** — ADR-0017이 22개로 만들었는데 여섯 곳이 20개로 남아 있었고, 보조 검사도 R-08까지로 적혀 있었다(실제 R-09). 합계 31건으로 통일

**뒤늦게 정리한 것**: 표준 E2E desktop 4건과 db 1건이 실패하고 있었다. **T6.4가 만든 것이 아니다** — main에서도 같은 자리에서 실패했다(직접 확인). 셋은 그 자리에서 고쳤고, 데모 콘텐츠 상태에 달린 나머지는 병행 세션(RPM)이 반입을 마친 뒤 원인이 드러나 함께 고쳤다.

| 스펙 | 원인 | 처리 |
|---|---|---|
| `route-builder` 게시 · `availability` 게시 | 개념을 붙이지 않은 개념 차시를 게시하려 한다. T2.4의 준비도 게이트가 **옳게** 거부한다(자료 없는 차시는 학생에게 빈 화면이다). 스펙이 게이트보다 먼저 쓰였다 | **고침** — 게시된 자료가 있고 선수 개념이 없는 개념(`일차방정식 복습`)을 붙였다 |
| `teacher-app` 스모크 | ① 명단 링크의 접근 이름이 비어 있다(이름·학년·점수가 각각 `<p>`). ② 문제은행에서 이름으로만 좁혀 첫 행을 잡는데, 교재 반입으로 「가감법」 문항이 85건이 되자 첫 행이 초안으로 바뀌었다(게시·사용 가능은 둘뿐) | **고침** — 명단은 구역으로 스코프하고, 문항은 화면의 검수 상태 필터(`?status=published`)를 건다. 문항이 얼마나 쌓이든 같은 것을 잰다 |
| `route-builder` 학생 오버라이드 | 오버라이드 폼은 그 학생이 속한 반의 활성 루트를 읽는데, `seed-unit1-demo`가 박서윤을 「1단원 검증반(중1)」에도 넣으면서 소속이 둘이 됐다. 폼이 중1 루트를 읽어 「건너뛸 노드가 기준 루트 버전에 없습니다」로 거부 | **고침** — 소속이 하나뿐인 학생(김시우)으로 옮겼다. 재는 것은 오버라이드 동작이지 반 선택이 아니다 |
| `materials` 자료 왕복 | 가감법 자료·진도 상태에 달렸다 | 반입 완료 후 **통과** |
| `curriculum-chain` (db) | 개념마다 문항 50개를 요구했는데, 그 50은 **성취기준 단위 합계**의 실측 하한을 개념 단위로 옮겨 온 값이었다. 실제 분포 46·66·75 중 하나가 걸렸다 | **고침** — 이 테스트는 사슬이 닿는지를 재므로 개념당 `>= 1`로 두고, 분량은 성취기준 단위 합계 50을 지키는 위 테스트에 남긴다 |

고친 방향은 하나다: **스펙을 데이터에 맞춰 약하게 만들지 않고, 스펙이 재려던 것을 재게 했다.** 첫 행을 잡던 것은 화면의 필터로, 소속이 둘인 학생은 하나인 학생으로, 개념 단위로 잘못 옮겨진 분량 기준은 사슬 도달 여부로. 분량 기준 자체는 그것이 원래 있던 자리(성취기준 단위)에 그대로 남아 있다.
...
- [x] 과거 완료 테스트로 오늘 완료가 되지 않는다. — T1.1·T1.3 (`core/learning/day-plan`, `attempt-date-gate.test.ts`)
- [x] 하루 계획과 항목이 서버에 영속화된다. — `learner_day_plans`·`_items` (T1.2). 첫 투영 경합까지 검증(`learner-day-plan.test.ts`)
- [x] 학생 화면과 DB 완료 상태가 일치한다. — 화면이 **서버가 확정한 계획**만 접는다(T1.4). 자율 E2E ⑤가 화면의 「모두 마쳤습니다」와 `completed_at`을 같은 흐름에서 본다

### Gate B — 실행 가능한 루트

- [x] 모든 필수 노드에 학생 실행기가 있다. — `NODE_EXECUTORS` 12종 전부 (T2.2)
- [x] 문항 0개·자료 미게시·권한 만료가 게시 전에 발견된다. — `checkRouteReadiness` (T2.4). 자율 E2E ②가 「자료 먼저, 그래야 게시」를 실제로 지난다
- [x] 숙제·교재 범위·평가 노드가 학생 행동으로 연결된다. — T2.1~T2.3

### Gate C — 자동 평가

- [x] 실제 워커가 날짜별 평가를 자동 생성한다. — 자율 E2E ④는 **클릭이 하나도 없는 테스트**다. 가짜 워커로 바꿔 실패를 확인했다 (T6.2)
- [x] 중복 생성, 워커 재시작, kill switch, DLQ 복구가 검증된다. — 중복 생성은 T6.3이 10겹으로(그 과정에서 **예외로 죽던 결함**을 찾아 고쳤다), 나머지는 T3.2·T3.4
- [x] 확인테스트 노드와 최신 숙련도·복습이 생성에 반영된다. — T3.3 (`scheduled-assessment-generation.test.ts`)

### Gate D — 닫힌 학습 순환

- [x] `LearnerDayCompleted`와 `SessionCompleted`가 구분되어 발행된다. — I-21 (T4.1·T4.2). 동시 호출에서도 각각 하나(T6.3)
- [x] 실제 진도와 숙련도가 미래 일정 변경안에 반영된다. — T4.3. 자율 E2E ⑦이 「못 나감」 마감 → 확인테스트 이동 → 승인 대기 1건까지 간다
- [x] 교사가 완료·막힘·승인 대기를 확인한다. — T4.4. 자율 E2E ⑥이 완료·기록 없음·막힘 0을 갈라 본다

### Gate E — 교사 사전 설정

- [x] 시드 없이 온보딩을 끝낼 수 있다. — 자율 E2E ①이 **빈 조직에서 설정 화면이 시키는 대로만** 따라간다 (T6.2). 그 과정에서 막다른 길 둘을 찾아 고쳤다
- [x] 담당 교사가 필요한 계정·루트·자료 작업을 권한 범위 안에서 수행한다. — T5.2·T5.3 (작업 단위 권한 + 담당 반 스코프)
- [x] 학생 로그인 전 readiness와 학생 화면 preview를 확인한다. — T5.4. 자율 E2E ③이 「아직 시험이 없는 날」을 교사가 학생보다 먼저 보는 것을 확인한다

### Gate F — 운영 투입

- [~] 서버 시험 마감·동시성·모바일·접근성 **통과** (T6.1·T6.3). **부하는 아직 재지 못했다** — k6 바이너리도 스테이징도 없다. 지금 재고 있는 것은 동시성(30명 동시 제출 유실 0·p95 623ms, 도메인 경로 한정)이지 규모가 아니다
- [x] 빈 워크스페이스→학생 하루 완료 실워커 E2E가 5회 연속 통과한다. — `--repeat-each=5` desktop 35/35 (T6.2). 이후 세 폭(desktop·tablet·mobile)으로 넓혀 23통과 (T6.3)
- [x] 모니터링·복구 런북·README·인수 현황이 실제 구현과 일치한다. — T6.4. RB-16 신설, kill switch 이름 다섯·불변 조건 개수 정합, `verify:recovery` 107위반 → 0
