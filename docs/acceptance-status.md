...
| 기호 | 뜻 | 개수 |
|---|---|---|
| ✅ | **검증됨** — 자동 테스트 또는 실행 검증이 존재하고 통과한다 | **23** |
| 🟡 | **구현됨·부분 검증** — 코드는 있으나 전용 테스트가 없거나 일부 조건만 만족한다 | **33** |
...
| 48 | 성취기준→문항 추적 사슬 | 🟡 | 사슬 앞 두 고리 **완전 커버** — 성취기준 60개(원문·체크섬) 전부에 사람 큐레이션 카탈로그(`middle-math-concept-catalog.mts`, 개념 69개·1:1 등치 아님 — 연립방정식은 1→4로 갈라지고 곱셈·인수분해는 2가 나눠 맡음)가 잇겼고, 커버리지 60/60을 테스트가 상시 단언. 문항까지 잇긴 성취기준 4개([9수01-01] 104문항 등 — 반입 문항 보유분만큼 자람). **학습 목표·평가 증거·블루프린트 도메인은 여전히 없다** — 사슬 뒤 세 고리가 다음 단계 |
