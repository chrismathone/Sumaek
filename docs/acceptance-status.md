...
| 기호 | 뜻 | 개수 |
|---|---|---|
| ✅ | **검증됨** — 자동 테스트 또는 실행 검증이 존재하고 통과한다 | **25** |
| 🟡 | **구현됨·부분 검증** — 코드는 있으나 전용 테스트가 없거나 일부 조건만 만족한다 | **32** |
...
**"앱이 스스로 데이터를 만들 수 없다"**는 문장은 더 이상 사실이 아니다 — 반·학습자 등록(설정), 루트 작성·검증 게이트·게시(빌더), 일정 실체화, 불참·휴강 접수→재계산, 학생 오버라이드, 개인정보 삭제 요청, 학습자 개별 일정 실체화까지 E2E(desktop) 39건이 완주한다(시드된 조직·교사 계정·과정 기간 위에서). **운영 안전장치**도 표시가 아닌 집행이 됐다: kill switch가 워커 클레임을 실제로 막고(작업은 큐에 보존), 일정 검증 실패는 이전 리비전을 유지하며, 낙관적 잠금이 VERSION_CONFLICT를 실제로 던지고, 회로 차단기·AI 비용 한도·DLQ 재처리·복구 검증 하네스(불변 29검사, 위반 0행)가 실DB에서 돈다.
...
**학습자 스코프 일정이 제품 안에 들어왔다**(인수 4). 계산·저장은 이미 있었지만 부르는 곳이 없어 인수 37과 같은 형태의 죽은 코드였다. 배선하면서 세 가지가 함께 드러났다. (1) **재합류는 화면에서 만들 수 없었다** — `createOverride`가 `rejoin_node_id`를 저장하지 않아 `is_rejoin`이 제품 경로에서는 영원히 false였다(표에 칸만 있고 값이 안 생기는 상태). 폼에 재합류 지점 선택을 추가해 닫았다. (2) **E2E 정리 루프가 한 번도 돈 적이 없었다** — 학생 상세로 이동한 직후 취소 버튼 수를 세어 아직 목록 화면인 채로 0을 읽고 곧장 break했고, 그래서 `toHaveCount(0)`도 통과했다. 시드 학습자에게 활성 오버라이드가 **31건** 쌓여 학생 경로가 보충 차시로만 채워질 지경이었다. 렌더 대기를 넣어 고치고, `purgeTestData`에 사유 규칙 기반 정리를 추가해 재발을 막았다. (3) **소비자 없는 토픽 2건**(`assessment.exclude-question`·`curriculum.impact-analysis`)이 `EVENT_CONSUMERS`에 있는데 워커에 핸들러가 없다. 작업이 `queued`로 남으므로 유실은 아니지만 아무도 하지 않는다 — 이번 범위 밖이라 고치지 않고 워커 테스트의 알려진 결손 목록에 명시했다(새 결손이 늘면 테스트가 걸린다). **(2026-08-02 해소: 둘 다 라우팅표에서 제거했다. 문항 격리의 신규 출제 제외는 출제 풀 질의(`review_status='published'`)와 불변 조건 I-13이 이미 집행하고, 개정 영향 분석은 구현된 적이 없으며 두 이벤트 모두 발행부가 0건이다. 근거는 `packages/db/src/queue.ts` 주석.)**

**교사↔학생 연동을 실제로 왕복시켜 보고 끊긴 곳을 고쳤다**(2026-08-02). 브라우저로 교사가 테스트를 생성·게시 → 학생 화면에 즉시 표시 → 응시·제출 → 자동 채점 → 교사의 점수·응시 이력·개념 숙련도까지 한 바퀴가 도는 것을 확인했다(전부 인라인, 워커 불필요). 그 과정에서 드러난 결손 여섯을 닫았다. (1) **답안 저장에 학습자 스코프가 없었다** — `saveResponse`가 `organization_id`만 봐서 같은 학원의 다른 학생 attemptId를 알면 그 응시에 답안을 써 넣을 수 있었다(제출 경로는 처음부터 `learner_id`를 검사했다). 둘을 같은 기준으로 맞췄다. (2) **등록한 학생이 로그인할 수 없었다** — `learners.user_id`를 채우는 제품 경로가 시드 한 줄뿐이라, 화면으로 등록한 학습자는 학생 앱에 들어올 수단이 없었다. 학생 상세에 계정 연결·발급·해제를 붙였다(소유자 전용, 초기 비밀번호는 한 번만 표시하고 저장하지 않는다). (3) **미래 테스트를 오늘 풀 수 있었다** — 생성 폼의 기본 날짜가 "다음 수업일"이라 미래 테스트가 기본 동작인데 게이트가 없었다(실측: 8/5 테스트를 8/2에 제출). 화면과 `startAttempt` 양쪽에 넣었다. (4) **복습이 영원히 쌓이기만 했다** — `review_items`에 insert만 있고 완료 경로가 0건이었다. 같은 개념을 나중에 맞히면 밀린 복습이 닫힌다. (5) **학생이 오늘 무엇을 배우는지 볼 수 없었다** — `/learn/**`이 읽는 테이블에 일정이 하나도 없었다. `/learn/today`에 오늘 차시를 넣되 **개별 일정을 먼저** 보고 없으면 반 공통으로 물러선다. (6) **제한 시간이 표시만 되고 강제되지 않았다** — 데모 러너에는 카운트다운이 있는데 실제 러너에는 없었다. 서버 `started_at` 기준 카운트다운과 자동 제출을 붙였다(**서버는 여전히 마감을 검사하지 않는다** — 컴포넌트 주석에 결손으로 남겼다). 부수로 `not-found.tsx`·`error.tsx`가 저장소에 0건이던 것(`notFound()`는 7곳에서 호출), `/app/no-access`가 실제 권한을 다시 보지 않아 owner에게 거짓을 말하던 것, 업무함·교재 목록이 바깥 스크롤을 만들던 것(브라우저 실측으로 행 수 재조정), 채점·오버라이드·불참 화면에 남아 있던 인라인 결과 문구를 알림으로 옮긴 것을 함께 고쳤다.

남은 ❌ 3건의 공통 뿌리였던 **교육과정 권위 실데이터가 2026-08-03에 들어왔다** — `pnpm curriculum:collect`가 교육부 고시 원문에서 중학교 성취기준 60개를 체크섬과 함께 적재하고, RPM 1단원 문항 213개까지 사슬이 실측으로 잇긴다. 같은 날 그 위에 셋이 섰다: **발행 게이트가 파이프라인이 됐고**(43 — 실릴리스의 남은 차단 사유는 사람 원문 대조 1건뿐), **정렬 사슬이 끝까지 잇겼고**(48 ✅ — 목표·증거·블루프린트, 죽은 자리였던 assessment_blueprints 배선), **수직 진행 탐색 화면이 섰다**(45 ✅ — 초등→고등 계통 + 표상·오개념). 남은 ❌는 **릴리스 diff 엔진(50)** 하나다.
...
| 17 | 빌드·린트·테스트 통과 | 🟡 | **린트 결손 해소(2026-08-01)** — `eslint.config.mjs`(flat, typescript-eslint + react-hooks + @next/next) 신설, `pnpm lint` 실동작(0 오류). CI도 생겼다 — `.github/workflows/ci.yml`이 boundary→lint→typecheck→test→build를 돌리며 시크릿 없이 통과함을 실측했다. 단위·통합 637건, E2E(desktop) 39건 통과(2026-08-02 실측). **접근성 자동 검사(axe)는 여전히 0건** |
...
| 41 | 교육과정 원문 역추적 | ✅ | `pnpm curriculum:collect`(2026-08-03) — 교육부 고시 제2022-33호 별책8을 직링크로 받아 sha256 기록, HWP 파서(`packages/ingest/python/hwp-text.py`)로 **중학교 성취기준 60개를 원문 자구 그대로** 적재 (이상 0건). 라이브 DB 테스트 6건(`curriculum-chain.test.ts`) — 전 성취기준의 체크섬 역추적·자구 일치 표본 포함. 커리큘럼 스튜디오에 원문·체크섬·릴리스 상태 노출. **사람 원문 대조(verified)는 절차가 도구화됨** — `pnpm curriculum:release verify-source`가 체크섬 앞자리 대조 입력·검증자 기록·감사를 강제(수행 자체는 사람의 일로 남김). 발행 게이트는 인수 43에 연결됨. registered·parsed 상태를 화면에 정직 표기 |
| 42 | 2015·2022 미혼재 | 🟡 | 실데이터 진입 — `curriculum_applicabilities`에 2026 중1·중2=2022 개정 적재 + 테스트가 "중3(2015)은 버전 데이터가 없어 비워 둔다"는 규칙까지 단언. **2015 개정 버전·화면의 버전 구분 표시·혼재 시나리오 테스트는 여전히 없음** |
| 43 | 선수 그래프 릴리스 차단 | ✅ | **파이프라인 연결(2026-08-03)** — 게이트 함수(`curriculum/graph.test.ts` 11건)가 실제 발행 전이에 연결됐다: `domain/curriculum-release.ts` `publishCurriculumRelease`가 순환·고아·근거 없는 개념·AI 위장·중복 코드에 **원문 verified·매핑 커버리지·kill switch(curriculum_release 직접 집행 — 미연결이던 키가 집행됨)**를 더해 전부 통과해야 parsed→published 전이. 실패 리포트는 릴리스 `validation_report`에 남고 스튜디오 화면에 표시. CLI `pnpm curriculum:release`(status·verify-source·publish --dry-run). 라이브 DB 12건(`curriculum-release-publish.test.ts`) + **변이 3종**(원문 게이트 제거→2 실패, kill switch 게이트 제거→1, 전이 무조건화→8) 원복 후 전량 통과. 실릴리스 dry-run 실측: 게이트가 RPM 개념 5종의 빈 근거를 실제로 잡아냄(카탈로그 근거 백필로 수리) — 남은 차단 사유는 **사람 원문 대조 1건뿐** |
...
| 45 | 수직 진행 탐색 화면 | ✅ | **화면·데이터 동시 진입(2026-08-03)** — `/app/content/curriculum/progression`: 개념을 고르면 이전 학교급(초등 선수 사슬, 재귀 CTE)부터 다음 확장(중3→고등 extends·transfer_to 사슬)까지 수직으로 서고, 표상·대표 오개념(오류 패턴·혼동 대상·교정 전략)·학습 목표가 함께 보인다. 승인 간선만 사용(AI 제안 제외 — planningEdges와 같은 방향), 내부 해석 고지(2K-3). 데이터: `middle-math-vertical-catalog.mts` — 인접 개념 3(초등 2·고등 1)·간선 15(소인수분해·방정식 두 계통 + gcd↔lcm 대조쌍)·표상 17·오개념 11 (문항 잇긴 10종 전부 커버). 검증: DB 2건(수직 사슬 4간선·학교급 실차이·표상·오개념 전수·혼동 교차 참조) + E2E(`teacher-app.spec.ts` — 초등→고등이 **계통 섹션 안에** 보이고 계통 이동 동작). **변이 검증이 테스트 결함을 잡았다**: 재귀 깊이를 1로 잘라도 E2E가 통과 — 단언이 상단 내비 링크에 걸리고 있었다. 계통 섹션으로 스코프해 고치니 같은 변이가 실패, 원복 후 통과. 한계: 계통 데이터는 두 계통 중심(전 개념 아님), 다이어그램이 아닌 수직 리스트, axe 스캔 미포함 |
...
| 48 | 성취기준→문항 추적 사슬 | ✅ | **사슬 완결(2026-08-03)** — ① 앞 두 고리: 성취기준 60개(원문·체크섬) 전부에 개념 69개 커버리지 60/60. ② 뒤 고리: 학습 목표 14·평가 증거 18(`middle-math-objective-catalog.mts` — 관찰 가능한 수행 서술 + 성공 증거·**허용 가능한 오류** 필수, 2M)을 **문항까지 잇긴 개념 10종**에 큐레이션. ③ 죽은 자리였던 `assessment_blueprints`(쓰는 곳 0곳)가 제품 경로에 배선 — 일일·확인테스트 생성이 블루프린트를 만들어 인스턴스에 잇고 목표·증거를 스냅샷하며, **목표 없는 개념은 objectiveGaps로 정직 기록**(확인테스트는 anchor_spec에 앵커 커버·공백 기록). 검증: `curriculum-chain.test.ts` 9건(5고리 왕복 — [9수01-01] 개념 전부 목표·증거·문항 보유) + `blueprint-chain.test.ts` 라이브 4건, **변이 2종**(blueprint_id 절단→3 실패, 목표 스냅샷 절단→2 실패) 원복 후 통과. 한계: 목표는 문항 보유 개념 10종뿐(반입이 늘면 카탈로그도 늘린다), 블루프린트를 보여 주는 화면은 없다 |
...
3. ~~**워커를 띄운 채로 도는 왕복 검증**~~ (2026-08-02 해소) — 조직 스코프 디스패처(`dispatchOutbox(sql, { organizationId })`)를 만들어 공유 DB에서도 남의 이벤트를 건드리지 않고 부를 수 있게 했다. `apps/worker/test/wiring/outbox-roundtrip.test.ts`(11건)가 **한 프로세스에서** outbox→디스패처→작업→핸들러→완료·Inbox까지 실제로 돈다. 루프 한 회차(`runOnce`)의 실패·연기 분기도 함께 덮는다. **남은 것**: 워커 *프로세스*를 실제로 띄운 상태의 E2E는 여전히 없다(전역 디스패치는 밀린 이벤트를 한 번에 배수한다 — `docs/simulation-scenario.md` 0.4).
