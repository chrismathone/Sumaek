...
| 41 | 교육과정 원문 역추적 | ✅ | `pnpm curriculum:collect`(2026-08-03) — 교육부 고시 제2022-33호 별책8을 직링크로 받아 sha256 기록, HWP 파서(`packages/ingest/python/hwp-text.py`)로 **중학교 성취기준 60개를 원문 자구 그대로** 적재 (이상 0건). 라이브 DB 테스트 6건(`curriculum-chain.test.ts`) — 전 성취기준의 체크섬 역추적·자구 일치 표본 포함. 커리큘럼 스튜디오에 원문·체크섬·릴리스 상태 노출. **사람 원문 대조(verified)와 릴리스 발행 게이트는 별도 절차로 남음**(registered·parsed 상태를 화면에 정직 표기) |
