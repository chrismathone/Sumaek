...
| 기호 | 뜻 | 개수 |
|---|---|---|
| ✅ | **검증됨** — 자동 테스트 또는 실행 검증이 존재하고 통과한다 | **24** |
...
남은 ❌ 3건의 공통 뿌리였던 **교육과정 권위 실데이터가 2026-08-03에 들어왔다** — `pnpm curriculum:collect`가 교육부 고시 원문에서 중학교 성취기준 60개를 체크섬과 함께 적재하고, RPM 1단원 문항 213개까지 사슬이 실측으로 잇긴다(48 ❌→🟡). 이제 남은 ❌ 2건은 그 데이터 **위에 세울 것들**이다: 수직 진행 탐색 화면(45 — 표상·오개념 데이터 포함), 릴리스 diff 엔진(50). 검사·화면·발행 게이트의 골조는 전부 준비되어 있다.
